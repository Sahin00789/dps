...
function Edituser({id, closewndo}) {
    ...
    const getUserbyid = async ()=>{
        const res = await axios.get(`http://127.0.0.1:5000/students/ukg/${id}`)
  ...
  useEffect(()=>{
    ...
    const handleForm = (e) => {
      ...
      const handleSubmit = async (e) => {
        e.preventDefault();
      const response = await axios.put(`http://127.0.0.1:5000/students/ukg/${id}`, user);
      
      if (response.status===200) {
         toast("Student Details Updated Succesfully");
         closewndo();
