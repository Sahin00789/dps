...
export default function marks({ id, closewndo }) {
  ...
  const getUserbyid = async () => {
    const res = await axios.get(`http://127.0.0.1:5000/students/three/${id}`);
  ...
  const onSave = async (id) => {
    const doc = await axios.post(
      `http://127.0.0.1:5000/students/three/${id}`,
      marks
    );
    console.log(doc);
    if (doc.status === 200) {
        ...
        <div className="edit-box">
          ...
          <div className="total">
            <p>{(marks.fBengali != "AB" ? parseInt(marks.fBengali) : 0) +
              ...
              (marks.fGeography != "AB" ? parseInt(marks.fGeography) : 0)+
              (marks.fHistory != "AB" ? parseInt(marks.fHistory) : 0)+
